...
export default function LeilaoList({ leiloes, isLoading }: LeilaoListProps) {
  if (isLoading) {
    return (
      ...
      {leiloes.map((leilao) => (
        ...
        >
          ...
          <p className="text-sm text-[#656565] mb-3 line-clamp-2">
            {leilao.descricao?.trim() || "Sem descrição"}
          </p>
          ...
          <div className="mb-3 pb-3 border-b border-[#F2F2F2]">
            ...
            <p className="text-lg font-bold text-[#635EF2]">
              R$ {formatCurrency(leilao.lanceMinimo)}
            </p>
          </div>
          ...
          {/* Ações */}
          <button className="w-full px-3 py-2 text-sm bg-[#635EF2] text-white rounded hover:bg-[#4A47B5] transition">
            Participar
          </button>
        </div>
      ))}
